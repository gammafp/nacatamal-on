// Import tests from the Math Vec2 module

import { Vec2, Vec2Add, Vec2Set, Vec2ToArray } from "nacatamal-on/Math/Vector2";
...
describe("Vec2 - tests", () =>
{
    test('New Vec2', () =>
    {
        const test1 = Vec2(3, 6);
        expect(test1).toMatchObject({ x: 3, y: 6 });
        const test2 = Vec2();
        expect(test2).toMatchObject({x: 0, y: 0});
        const test3 = Vec2(4, 5);

        const test4 = Vec2Set(test3);

        expect(test4).toMatchObject(test3);

    });

    test("Add Vector2", () => {
        const test1 = Vec2(3, 6);
        const test2 = Vec2(4, 5);
        const test3 = Vec2(7, 11);

        const test = Vec2Add(test1, test2);

        expect(test).toMatchObject(test3);
    })

    test('Vec2 to array', () =>
    {
        const vec2 = Vec2(3, 6);
        const result = Vec2ToArray({y: 6, x: 3});
        const match = [3, 6];
